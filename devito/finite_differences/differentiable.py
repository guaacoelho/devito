...
class Differentiable(sympy.Expr, Evaluable):
    ...
    def has(self, *pattern):
        ...
        as fallback.
        """
        if isinstance(pattern[0], type) and issubclass(pattern[0], sympy.Symbol):
            ...
            return any(isinstance(i, pattern[0]) for i in self.free_symbols)
        return super(Differentiable, self).has(*pattern)
